...
namespace WPFitter\Aws\Credentials;

use WPFitter\Aws\Configuration\ConfigurationResolver;
use WPFitter\Aws\Exception\CredentialsException;
use WPFitter\Aws\Exception\InvalidJsonException;
use WPFitter\Aws\Sdk;
use WPFitter\GuzzleHttp\Exception\TransferException;
...
class InstanceProfileProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->client = $config['client'] ?? \WPFitter\Aws\default_http_handler();
    ...
    private function request($url, $method = 'GET', $headers = [])
    {
        ...
        $userAgent .= ' ' . \WPFitter\Aws\default_user_agent();
    ...
    {
        $isImdsV1Disabled = \WPFitter\Aws\boolean_value($this->ec2MetadataV1Disabled) ?? \WPFitter\Aws\boolean_value(ConfigurationResolver::resolve(self::CFG_EC2_METADATA_V1_DISABLED, self::DEFAULT_AWS_EC2_METADATA_V1_DISABLED, 'bool', $this->config)) ?? self::DEFAULT_AWS_EC2_METADATA_V1_DISABLED;
