...
use WPFitter\Aws\Api\ApiProvider;
use WPFitter\Aws\Api\Service;
use WPFitter\Aws\Api\Validator;
use WPFitter\Aws\Auth\AuthResolver;
use WPFitter\Aws\Auth\AuthSchemeResolver;
use WPFitter\Aws\Auth\AuthSchemeResolverInterface;
use WPFitter\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use WPFitter\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use WPFitter\Aws\ClientSideMonitoring\Configuration;
use WPFitter\Aws\Configuration\ConfigurationResolver;
use WPFitter\Aws\Credentials\CredentialProvider;
use WPFitter\Aws\Credentials\Credentials;
use WPFitter\Aws\Credentials\CredentialsInterface;
use WPFitter\Aws\DefaultsMode\ConfigurationInterface as ConfigModeInterface;
use WPFitter\Aws\DefaultsMode\ConfigurationProvider as ConfigModeProvider;
use WPFitter\Aws\Endpoint\EndpointProvider;
use WPFitter\Aws\Endpoint\PartitionEndpointProvider;
use WPFitter\Aws\Endpoint\UseDualstackEndpoint\Configuration as UseDualStackEndpointConfiguration;
use WPFitter\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface as UseDualStackEndpointConfigurationInterface;
use WPFitter\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider as UseDualStackConfigProvider;
use WPFitter\Aws\Endpoint\UseFipsEndpoint\Configuration as UseFipsEndpointConfiguration;
use WPFitter\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface as UseFipsEndpointConfigurationInterface;
use WPFitter\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider as UseFipsConfigProvider;
use WPFitter\Aws\EndpointDiscovery\ConfigurationInterface;
use WPFitter\Aws\EndpointDiscovery\ConfigurationProvider;
use WPFitter\Aws\EndpointV2\EndpointDefinitionProvider;
use WPFitter\Aws\Exception\AwsException;
use WPFitter\Aws\Exception\InvalidRegionException;
use WPFitter\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use WPFitter\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use WPFitter\Aws\Signature\SignatureProvider;
use WPFitter\Aws\Token\Token;
use WPFitter\Aws\Token\TokenInterface;
use WPFitter\Aws\Token\TokenProvider;
...
class ClientResolver
{
    ...
    {
        if ($value === \false) {
            $value = new Configuration(\false, \WPFitter\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \WPFitter\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \WPFitter\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
    ...
    public static function _apply_endpoint_provider($value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            if ($value instanceof \WPFitter\Aws\EndpointV2\EndpointProviderV2) {
            ...
            $args['region'] = \WPFitter\Aws\strip_fips_pseudo_regions($args['region']);
    ...
    public static function _apply_user_agent($inputUserAgent, array &$args, HandlerList $list)
    {
        ...
        if (isset($args['endpoint_discovery'])) {
            if ($args['endpoint_discovery'] instanceof \WPFitter\Aws\EndpointDiscovery\Configuration && $args['endpoint_discovery']->isEnabled()) {
        ...
        if (isset($args['retries'])) {
            if ($args['retries'] instanceof \WPFitter\Aws\Retry\Configuration) {
    ...
    public static function _default_endpoint_provider(array $args)
    {
        ...
        if (self::isValidService($serviceName) && self::isValidApiVersion($serviceName, $apiVersion)) {
            ...
            return new \WPFitter\Aws\EndpointV2\EndpointProviderV2($ruleset, EndpointDefinitionProvider::getPartitions());
        }
        $options = self::getEndpointProviderOptions($args);
    ...
    public static function _apply_suppress_php_deprecation_warning($value, &$args)
    {
        if ($value) {
            ...
            $args['suppress_php_deprecation_warning'] = \WPFitter\Aws\boolean_value(\getenv("AWS_SUPPRESS_PHP_DEPRECATION_WARNING"));
        } elseif (!empty($_SERVER["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"])) {
            $args['suppress_php_deprecation_warning'] = \WPFitter\Aws\boolean_value($_SERVER["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"]);
        } elseif (!empty($_ENV["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"])) {
            $args['suppress_php_deprecation_warning'] = \WPFitter\Aws\boolean_value($_ENV["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"]);
        }
    ...
    public static function _default_endpoint(array &$args)
    {
        ...
        $serviceIdentifier = \WPFitter\Aws\manifest($args['service'])['serviceIdentifier'];
...
EOT;
    ...
    private static function isValidService($service)
    {
        ...
        $services = \WPFitter\Aws\manifest();
