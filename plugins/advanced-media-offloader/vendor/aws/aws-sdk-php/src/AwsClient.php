...
use WPFitter\Aws\Api\DocModel;
use WPFitter\Aws\Api\Service;
use WPFitter\Aws\Auth\AuthSelectionMiddleware;
use WPFitter\Aws\Auth\AuthSchemeResolverInterface;
use WPFitter\Aws\EndpointDiscovery\EndpointDiscoveryMiddleware;
use WPFitter\Aws\EndpointV2\EndpointProviderV2;
use WPFitter\Aws\EndpointV2\EndpointV2Middleware;
use WPFitter\Aws\Exception\AwsException;
use WPFitter\Aws\Signature\SignatureProvider;
use WPFitter\GuzzleHttp\Psr7\Uri;
...
class AwsClient implements AwsClientInterface
{
    ...
    {
        $list = $this->getHandlerList();
        $list->prependBuild(AuthSelectionMiddleware::wrap($this->authSchemeResolver, $this->getApi()), 'auth-selection');
    }
    ...
    {
        $aliases = \WPFitter\Aws\load_compiled_json(__DIR__ . '/data/aliases.json');
